...
POPULAR_KEYS_FILE="top-images.txt"   # CloudWatch 로그 기준 상위 자산 목록
MAX_WARM_REQUESTS=500                # 배포당 워밍 상한
count=0

while read -r file; do
  for size in "${POPULAR_SIZES[@]}"; do
    if [ "$count" -ge "$MAX_WARM_REQUESTS" ]; then
      break 2
    fi
