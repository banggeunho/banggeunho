...
3. [아키텍처 설계: OpenSearch를 선택한 이유](#아키텍처-설계-opensearch를-선택한-이유)
4. [OpenSearch 전환 결과](#opensearch-전환-결과)
...
Fuse.js의 한계를 분석한 결과, 새로운 검색 시스템에 필요한 요건이 명확해졌습니다.

- **역 인덱스 기반 검색**: 전체 데이터 순회가 아닌, 상품 수가 늘어도 일정한 성능을 유지하는 구조
- **검색 전용 클러스터 분리**: API 서버의 메모리/CPU를 검색이 잠식하지 않도록 검색 워크로드를 별도 인프라로 격리
- **운영 독립성**: 동의어 추가, 검색 순위 조정 등을 코드 배포 없이 운영팀이 직접 수행

자체 Elasticsearch를 구축하는 방안도 검토했지만, 클러스터 운영(노드 교체, 버전 업그레이드, 스냅샷 관리)에 드는 운영 비용이 작은 팀 규모에 맞지 않았습니다. 이미 ECS, Lambda, Redshift 등 AWS 기반으로 인프라를 운영하고 있었기 때문에, AWS 관리형 검색 서비스를 사용하는 것이 운영 부담을 줄이면서도 기존 인프라와 자연스럽게 통합할 수 있는 선택이었습니다.

### OpenSearch vs CloudSearch

AWS 관리형 검색 서비스는 Amazon CloudSearch와 Amazon OpenSearch Service 두 가지가 있었습니다.
...
검색 아키텍처와 별도로, DB의 상품 데이터를 OpenSearch와 동기화하고 검색 품질을 관리하는 파이프라인을 구성했습니다. 상품 동기화는 **전체 색인**(1시간 주기 배치)과 **부분 색인**(실시간 단건 업데이트) 두 가지 전략을 조합하여, 데이터 정합성과 실시간성을 모두 확보했습니다.

![색인 및 사전 업데이트 아키텍처](images/색인_사전_파이프라인_v2.png)

- **상품 부분 색인 (Lambda + SQS)**: 어드민 및 셀러오피스에서 상품 정보를 수정하면, 변경 이벤트가 SQS 큐로 전송되고 Lambda가 해당 상품만 즉시 재색인합니다. 전체 색인 주기(1시간)를 기다리지 않고 실시간에 가까운 검색 데이터 갱신이 가능합니다.
...
어드민이나 셀러오피스에서 상품 정보(가격, 상품명, 카테고리 등)를 수정하면, 변경 이벤트가 SQS 큐로 전송되고 Lambda가 트리거되어 해당 상품만 OpenSearch에 즉시 반영합니다.

```
1. 어드민/셀러오피스에서 상품 정보 수정
2. API 서버가 수정 이벤트를 SQS 큐로 전송
3. SQS가 부분 색인 Lambda 트리거
4. Lambda가 DB에서 해당 상품의 최신 데이터 조회
5. OpenSearch에 단건 Update (Index API)
```

```typescript
// 부분 색인 Lambda - SQS 이벤트 핸들러
export const handler = async (event: SQSEvent) => {
  for (const record of event.Records) {
    const { itemId } = JSON.parse(record.body);

    // DB에서 최신 상품 데이터 조회
    const item = await getItemById(dbConnection, itemId);
    if (!item) continue;

    // 태그, 리뷰 등 부가 데이터 조회
    const [tags, reviews, recommendRatings] = await Promise.all([
      getItemTags([item], dbConnection),
      getReviews([item], dbConnection),
      getRecommendRatings([item], dbConnection)
    ]);
...
부분 색인 실패에 대비하여 별도의 **DLQ(Dead Letter Queue)**를 구성해 두었지만, 전체 색인이 1시간 주기로 돌기 때문에 부분 색인에서 일시적 오류가 발생하더라도 최대 1시간 내에 전체 색인이 데이터를 다시 맞춰줍니다. 이러한 구조적 안전망 덕분에 부분 색인의 예외 처리는 기본적인 수준으로 유지하면서도 데이터 정합성을 보장할 수 있었습니다.

**상품 전체 색인 플로우:**

부분 색인이 실시간 변경을 처리하는 반면, 전체 색인은 CTR 데이터 갱신, 신규 상품 일괄 반영 등 배치성 작업을 담당합니다. 색인 Lambda는 단순히 DB를 복사하는 것이 아니라, 여러 소스의 데이터를 조합하여 검색에 최적화된 문서를 생성합니다.
...
while (true) {
  ...
  await opensearchClient.bulk({ body: bulkOps });
}
```

**동시 색인 방지:**

전체 색인과 부분 색인이 동시에 실행되면 데이터 유실이 발생할 수 있습니다. 부분 색인은 Alias를 통해 현재 인덱스에 쓰는데, 전체 색인이 새 인덱스를 만들고 Alias를 전환하면 이전 인덱스에 쓰인 부분 색인 내용이 유실됩니다. 전체 색인이 DB에서 읽어오기 때문에 대부분의 경우 새 인덱스에 최신 데이터가 반영되지만, 전체 색인이 해당 상품을 이미 읽은 뒤에 수정이 발생한 경우에는 새 인덱스에도 반영되지 않는 엣지 케이스가 존재합니다.

이를 방지하기 위해 전체 색인 실행 중에는 부분 색인을 일시 중단하는 방식을 적용했습니다. 전체 색인 Lambda가 시작되면 SQS Lambda 트리거를 비활성화하고, 전체 색인이 완료되어 Alias가 전환된 후 트리거를 다시 활성화합니다. 그 사이 발생한 상품 수정 메시지는 SQS 큐에 안전하게 쌓여 있다가, 트리거 재활성화 후 새 인덱스에 반영됩니다. 전체 색인 소요 시간이 수 분 정도이므로 사용자 체감 영향은 거의 없습니다.

이 방식으로 **부분 색인과 전체 색인을 조합하여 상품 데이터 동기화**를 구성했습니다. 부분 색인이 실시간 변경을 즉시 반영하고, 전체 색인이 CTR 등 배치 데이터를 주기적으로 갱신하며, 동시 실행 방지를 통해 두 전략 간의 데이터 정합성을 보장합니다.
...
하지만 여기서 멈추지 않았습니다. 300ms는 충분히 빠른 편이었지만, 실제 사용자 여정 기준으로는 검색 속도와 검색 품질(CTR 17%) 모두에 추가 개선 여지가 있었습니다. 이를 해결하기 위해 두 가지 핵심 고도화를 진행했습니다.
          ...
          "must": [
            {
              "multi_match": {
                "query": "나이키 운동화",
                "fields": ["product_name.nori^3", "brand^2", "combined_category_name^1.5"]
              }
            }
          ]
        }
...
- `default_ctr: 5.0` — `search_ctr`가 없는 문서(신규/누락 데이터)는 기본 CTR 5% 사용
...
- **그룹 A (기존)**: OpenSearch 검색 시스템
- **그룹 B (개선)**: OpenSearch + CTR 랭킹 검색 시스템

해당 A/B는 CTR 랭킹 단일 요소가 아니라 **검색 시스템 전환 전체 효과**를 측정한 결과입니다.
...
| **CTR** | 15%    | 22.5% | **+7.5%p ** | 세션 기반 A/B 테스트 (p < 0.01)                |
...
### 1. 검색 아키텍처 설계와 관심사 분리

검색 시스템의 전체 아키텍처를 직접 설계하면서, **관심사 분리**의 중요성을 체감했습니다. 검색을 별도 클러스터로 분리하여 메인 서버의 안정성을 확보하고, 캐시 레이어를 어디에 배치할지 판단하여 OpenSearch 부하를 줄이며, 색인 파이프라인을 검색 서빙과 독립적으로 구성하는 등 — 각 컴포넌트의 역할을 명확히 나누는 것이 성능과 안정성 모두에 직결된다는 것을 배웠습니다.

### 2. 사용자에게 영향 없는 데이터 파이프라인 구축

색인 파이프라인을 구축하면서 가장 많이 고민했던 부분은 **서비스 중단 없이 데이터를 안정적으로 동기화하는 방법**이었습니다. Alias 원자적 전환으로 색인 교체 중에도 검색이 끊기지 않게 하고, 부분 색인과 전체 색인의 역할을 분리하여 실시간성과 정합성을 동시에 확보하며, 동시 색인 방지로 데이터 유실을 막는 등 — 모든 설계 판단의 기준은 "사용자에게 영향 없이 안정적으로 동작하는가"였습니다.

### 3. 성능 최적화는 측정에서 시작된다

CTR 기반 랭킹, Redis 캐싱, 커서 기반 페이지네이션 등 여러 성능 개선을 진행했지만, 가장 중요했던 것은 **무엇이 병목인지 정확히 파악하는 것**이었습니다. OpenSearch 전환만으로 응답 속도는 해결되었지만 CTR은 여전히 낮았고, 검색 자체는 빨라졌지만 부가 조회가 전체 응답을 지연시키고 있었습니다. A/B 테스트와 p95 지표를 기준으로 실제 효과를 검증하며 개선 방향을 잡아간 경험이 이후 다른 프로젝트에서도 큰 도움이 되었습니다.
