...
카카오 i LaaS의 OMS(주문 관리 시스템)는 여러 외부 커머스의 주문을 통합 관리합니다. 위메프, 롯데ON, TMON 등 5개 커머스에서 발생한 주문을 매시간 수집하여 DB에 저장하고, 통합 대시보드로 제공합니다.

각 커머스별 배치는 **여러 판매자의 API Key**를 통해 주문 데이터를 수집합니다. 하나의 커머스에 수십 개의 판매자가 등록되어 있으며, 판매자마다 개별 API Key로 인증하여 주문을 조회합니다.

**비즈니스 요구사항:**
- 각 커머스의 주문 데이터를 5분마다 수집
- 통합 대시보드에서 실시간 현황 파악

하지만 주문이 20,000건 이상인 커머스 배치에 대해서는 약 30분이 걸렸고, 신규 커머스를 추가하려면 2주 이상 소요되던 문제가 있었습니다.
